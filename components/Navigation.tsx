  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 md:relative md:w-64 bg-slate-900 border-t md:border-t-0 md:border-r border-slate-800 p-4 z-50 flex flex-col">
      <div className="hidden md:block mb-8 px-4">
        ...
        <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Sincronizado</p>
      </div>
        ...
        {currentUser ? (
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-3">
              <img src={currentUser.avatar || 'https://via.placeholder.com/150'} className="w-10 h-10 rounded-xl border border-indigo-500/30 shadow-lg shadow-indigo-500/10" alt="Avatar" />
              <div className="overflow-hidden">
                <p className="text-xs font-black text-white truncate">{currentUser.username}</p>
                <p className="text-[9px] text-indigo-400 font-bold uppercase tracking-widest">En línea</p>
              </div>
            </div>
            <button onClick={onLogout} className="text-[9px] font-black text-slate-600 hover:text-red-400 uppercase tracking-widest transition-colors text-left">
              Cerrar Sesión
            </button>
          </div>
        ) : (
