            ...
            <div className="flex flex-col items-center group">
              <div className="relative mb-4">
                {top3[1].discordUser?.avatar ? (
                  <img src={top3[1].discordUser.avatar} className="w-16 h-16 md:w-24 md:h-24 rounded-2xl border-2 border-slate-400 shadow-xl object-cover" alt="P2" />
                ) : (
              ...
              <p className="text-slate-300 font-bold text-center text-xs truncate w-full px-2">{top3[1].playerName}</p>
            </div>
          ) : <div />}
            ...
            <div className="flex flex-col items-center group -translate-y-6">
              <div className="relative mb-4">
                ...
                {top3[0].discordUser?.avatar ? (
                  <img src={top3[0].discordUser.avatar} className="relative w-24 h-24 md:w-32 md:h-32 rounded-3xl border-4 border-yellow-500 shadow-2xl z-10 object-cover" alt="P1" />
                ) : (
              ...
              <p className="text-white font-black text-center text-sm truncate w-full px-2">{top3[0].playerName}</p>
              <p className="text-yellow-500 font-mono font-bold text-xs mt-1">{top3[0].maxDamage.toLocaleString()}</p>
            </div>
          )}
            ...
            <div className="flex flex-col items-center group">
              <div className="relative mb-4">
                {top3[2].discordUser?.avatar ? (
                  <img src={top3[2].discordUser.avatar} className="w-16 h-16 md:w-20 md:h-20 rounded-2xl border-2 border-amber-700 shadow-xl object-cover" alt="P3" />
                ) : (
              ...
              <p className="text-slate-300 font-bold text-center text-xs truncate w-full px-2">{top3[2].playerName}</p>
            </div>
          ) : <div />}
        </div>
      )}
        ...
        <table className="w-full text-left">
          <thead className="bg-slate-950/50 border-b border-slate-800">
            <tr>
              ...
              <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Jugador</th>
              <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest text-right">Daño Máximo</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-800/50">
            {stats.length > 0 ? stats.map((player, index) => (
                ...
                <td className="px-6 py-6">
                  <div className="flex items-center gap-4">
                    {player.discordUser?.avatar ? (
                      <img src={player.discordUser.avatar} className="w-10 h-10 rounded-xl border border-slate-800 object-cover" alt="Av" />
                    ) : (
                      <div className="w-10 h-10 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-center text-xs">⚔️</div>
                    )}
                    <div>
              ...
              <tr>
                <td colSpan={3} className="px-6 py-20 text-center text-slate-700 uppercase text-[10px] font-black tracking-widest opacity-30">
                  Esperando registros...
                </td>
              </tr>
            )}
